...
/** Request Host port wins so one `next build` can serve :3000, :3001, and :3002. */
...
function expressPortFromUrl(expressBaseUrl: string): string {
  let parsed: URL;
  try {
...
export function assertAppEnvIsolation(
  ...
  const port = expressPortFromUrl(expressBaseUrl);

  if (port !== current.expressPort) {
    const owner = APP_ENVS.find((key) => ROOMS[key].expressPort === port);
    const ownerLabel = owner ?? `port ${port}`;
    throw new Error(
      `[app-env] APP_ENV=${input.appEnv} refuses Express on port ${port} (${ownerLabel}). Use port ${current.expressPort}.`,
    );
  }
}
